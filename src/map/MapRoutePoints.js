...
// Função auxiliar para calcular a distância (em metros) entre dois pontos geográficos usando a fórmula de Haversine.
...
// Função auxiliar para calcular a média de dois ângulos, tratando a natureza cíclica (0° e 360°).
...
// Função que cria pontos fantasmas ao longo de cada segmento quando a distância é maior que o espaçamento desejado (padrão: 50 m).
const createGhostPositions = (positions, spacing = 50) => {
  ...
  for (let i = 0; i < positions.length - 1; i++) {
    ...
    // Adiciona o ponto original inicial.
    ghostPositions.push(start);

    // Calcula a distância do segmento entre start e end.
    ...
    // Se a distância for maior que o espaçamento, insere pontos fantasmas.
    if (d > spacing) {
      ...
      for (let j = 1; j < numGhosts; j++) {
        ...
        // Calcula o ângulo médio corretamente entre os dois pontos.
        ...
        const ghostPoint = {
          ...
          speed: ghostSpeed,
          course: ghostCourse,
          isGhost: true, // Flag para identificar que é um ponto fantasma.
        };
...
const MapRoutePoints = ({
  ...
  useEffect(() => {
    ...
    // Gera os pontos (originais + pontos fantasmas) com espaçamento uniforme de 50 metros.
    const processedPositions = createGhostPositions(positions, 50);

    // Se necessário, filtra os pontos com base no zoom para melhorar a performance ou visual.
    const finalPositions = needFilterPosition
      ? processedPositions.filter((_, index) =>
          index % (zoomLevel < 15 ? 16 : zoomLevel > 15 && zoomLevel < 16 ? 8 : 4) === 0
        )
      : processedPositions;

    // Calcula os valores mínimo e máximo de velocidade a partir dos pontos originais.
    const maxSpeed = positions.reduce((a, b) => Math.max(a, b.speed), -Infinity);
    const minSpeed = positions.reduce((a, b) => Math.min(a, b.speed), Infinity);
