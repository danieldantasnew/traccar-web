...
export const formatTime = (value, format) => {
  ...
  if (
    ...
    formattedDate = `Hoje às ${horas}:${minutos}:${segundos}`;
  } else if (
    d.getFullYear() === yesterday.getFullYear() &&
    d.getMonth() === yesterday.getMonth() &&
    d.getDate() === yesterday.getDate()
  ) {
    formattedDate = `Ontem às ${horas}:${minutos}:${segundos}`;
  } else {
    const dayOfWeek = daysOfWeek[d.getDay()];
    formattedDate = `${dayOfWeek} ${String(d.getDate()).padStart(2, "0")}/${String(d.getMonth() + 1).padStart(2, "0")}/${d.getFullYear()} às ${horas}:${minutos}:${segundos}`;
  }
...
export const formatDistance = (value, unit, t) =>
  `${distanceFromMeters(value, unit).toFixed(1)} ${distanceUnitString(
    unit,
    t
  )}`;
...
export const formatSpeedNoTranslation = (value, unit) =>
  `${Math.round(speedFromKnots(value, unit))} km/h`;
